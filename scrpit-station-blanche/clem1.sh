...
if [ -z "$USB_DEVICE" ]; then
    zenity --error --text="Aucune clé USB détectée." --timeout=3
    exit 1
fi
...
if [ ! -d "$MOUNT_POINT" ]; then
    zenity --error --text="Aucune clé USB montée à $MOUNT_POINT." --timeout=3
    exit 1
fi
...
for FILE in $FILES_TO_SCAN; do
    clamdscan --fdpass -r "$FILE" >> "$LOG_FILE"
...
if [ $? -ne 0 ]; then
    zenity --error --text="Analyse annulée par l'utilisateur." --timeout=3
    exit 1
fi
...
if [ -z "$INFECTED_FILE" ]; then
    zenity --info --text="Aucun virus détecté." --timeout=3
else
    zenity --error --text="Virus détecté dans les fichiers suivants :\n\n $INFECTED_FILE \n\n Suppression obligatoire." --timeout=10
    exit 1
fi
...
for file in $FILES_TO_COPY; do
        ...
        zenity --error --text="Fichier pas sur la clé usb" --timeout=3
        exit 1
fi
done

if [ -z "$FILES_TO_COPY" ]; then
    zenity --error --text="Aucun fichier sélectionné." --timeout=3
    exit 1
fi

cp $FILES_TO_COPY "$TEMP_DIR/"
zenity --info --text="Fichiers copiés dans le dossier temporaire : $TEMP_DIR" --timeout=3

# Attente d'une nouvelle clé USB
zenity --info --text="Retirez la clé et insérez une clé USB sécurisée. Appuyez sur OK"
udisksctl mount -b "$USB_DEVICE"
...
clamdscan --fdpass -r "$NEW_MOUNT_POINT"
# Vérification et copie vers la nouvelle clé
zenity --info --text="Nouvelle clé détectée à $NEW_MOUNT_POINT. Copie en cours..." --timeout=3
cp -r "$TEMP_DIR"/* "$NEW_MOUNT_POINT/"
zenity --info --text="Fichiers copiés avec succès." --timeout=3

# Nettoyage
rm -rf "$TEMP_DIR"
zenity --info --text="Nettoyage terminé. Processus finalisé." --timeout=3
zenity --info --text="Retirez la clé usb sécurisé. Merci et bonne continuation" --timeout=5
